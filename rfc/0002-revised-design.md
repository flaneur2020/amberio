...
- 示例：`s3://rimio-archive/slot-731/pack-00042?range=bytes=1048576-2097151`
...
RFC 0002 将 Rimio 的核心从“chunk/事务驱动”收敛为“对象路径驱动”：
...
- 认证（建议）：
  ...
  - 内部 API：mTLS + `X-Rimio-Node-Id`。
- 幂等键：`X-Rimio-Write-Id`（PUT/DELETE 建议必传）。
...
- `GET /api/v1/blobs/{blobPath}`
  ...
  - 返回头建议：
    - `ETag: <etag>`
    - `X-Rimio-Generation: <generation>`
    - `Content-Length`
  - 错误：`404 Not Found`（不存在）、`410 Gone`（被 tombstone 覆盖）。

- `HEAD /api/v1/blobs/{blobPath}`
  - 不返回 body，仅返回元信息头：`ETag`、`X-Rimio-Generation`、`Content-Length`。

- `DELETE /api/v1/blobs/{blobPath}`
  - 请求头：`X-Rimio-Write-Id`（建议）。
