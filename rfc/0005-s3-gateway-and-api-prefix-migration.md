...
> 引入 `rimio-s3-gateway` 包，提供 S3 兼容入口；同时将 Rimio 原生 REST API 统一迁移到 `/_/api/v1/*` 前缀。
...
2. Rimio 自身原生 REST API（`/api/v1/*`）应与 S3 路由解耦，避免顶层路径冲突。
...
1. 新增独立 crate：`rimio-s3-gateway`，承载 S3 协议解析与 XML 响应编排。
2. `rimio-server` 继续作为统一进程与端点暴露层，挂载：
   - Rimio 原生 API：`/_/api/v1/*`
   - Rimio 内部 API：`/internal/v1/*`（保持不变）
...
4. 本 RFC 不改变 Rimio 内部存储模型（slot/head/part）。
...
- bucket 仅作为逻辑命名空间，落地到 Rimio path 时映射为 `bucket/key`。
...
1. 使用 `boto3` 对 Rimio 发起 path-style S3 请求。
2. 覆盖场景：
   - PutObject
   - GetObject
   - HeadObject
...
1. 创建 `rimio-s3-gateway` crate，定义：
   - S3 请求参数解析
   - XML 响应模型
   - `GatewayBackend` trait
   - axum router 与 handlers
2. 在 `rimio-server` 实现 `GatewayBackend`，桥接 core operations。
3. `rimio-server` 路由迁移：
   - 原生 API 改到 `/_/api/v1/*`
   - 挂载 S3 路由到根路径
